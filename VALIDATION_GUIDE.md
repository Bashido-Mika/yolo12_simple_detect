# 🎯 YOLOモデル評価ガイド（統合版）

## 📋 概要

このガイドでは、整理・統合された新しい評価システムの使い方を説明します。

---

## 🚀 クイックスタート

### **1. Ultralyticsで公式精度を測定（推奨）**

```bash
uv run validate.py ultralytics --model runs/train/train12/weights/best.pt
```

**出力例:**
```
[Segmentation]
  mAP50-95: 0.3627 (36.27%) ← あなたのモデルは良好！
  mAP50:    0.6920 (69.20%)
  Precision: 0.6885
  Recall:    0.6710
```

### **2. SAHIでスライス推論の効果を測定**

```bash
uv run validate.py sahi --yolo-dataset Dataset/YOLODataset_test_with_label
```

### **3. 一度に両方を実行して比較（最もおすすめ）**

```bash
uv run validate.py compare \
  --yolo-dataset Dataset/YOLODataset_test_with_label \
  --error-analysis
```

**出力:**
```
[Ultralytics]
  Segm mAP50-95: 0.3627 (36.27%) ← 公式ベンチマーク（信頼できる）

[SAHI]
  Segm mAP50-95: 0.1290 (12.90%) ← 参考値（絶対値として使えない）

💡 SAHIの値が低いのは正常です（内部処理の違い）
💡 Ultralyticsの値を精度評価に使用してください
```

---

## 📁 新しいディレクトリ構造

```
yolo12_detect/
├── validate.py                      # ✨ 新しい統合CLIエントリーポイント
├── validation/                      # ✨ 整理されたモジュール
│   ├── __init__.py
│   ├── README.md                    # 詳細なドキュメント
│   ├── ultralytics_validator.py     # ✅ Ultralytics評価
│   ├── sahi_validator.py            # ✅ SAHI評価（ラッパー）
│   ├── comparator.py                # ✅ 比較ロジック
│   └── utils/
│       ├── coco_converter.py        # YOLO→COCO変換
│       ├── visualization.py         # 比較画像生成
│       └── metrics.py               # メトリクス計算
│
├── SAHI_val.py                      # 既存（互換性のため維持）
└── ultralytics_val.py               # 既存（互換性のため維持）
```

---

## 📝 データセット要件

### YAMLファイルの柔軟な検索

SAHIモードは以下の優先順位でYAMLファイルを自動検索します：

1. **data.yaml** （Ultralytics標準）
2. **dataset.yaml** （よくある代替名）
3. **任意の*.yamlまたは*.yml** （最初に見つかったもの）

```
Dataset/YOLODataset_xxx/
├── data.yaml          ✅ 推奨
├── dataset.yaml       ✅ OK
├── config.yaml        ✅ OK（他にない場合）
├── images/
│   └── val/
└── labels/
    └── val/
```

### YAML形式

**辞書形式（推奨）**:
```yaml
names:
  0: class1
  1: class2
  2: class3
nc: 3  # オプション（自動計算可）
train: images/train
val: images/val
```

**リスト形式（対応済み）**:
```yaml
names: [class1, class2, class3]
nc: 3  # オプション（自動計算可）
train: images/train
val: images/val
```

---

## 🎓 詳細な使い方

### **モード1: Ultralytics標準評価**

```bash
# 基本
uv run validate.py ultralytics

# カスタム設定
uv run validate.py ultralytics \
  --model runs/train/train12/weights/best.pt \
  --name my_validation \
  --imgsz 640 \
  --conf 0.25 \
  --iou 0.6
```

**いつ使う?**
- ✅ 公式ベンチマークと比較したい
- ✅ 絶対的な精度を知りたい
- ✅ 速く正確に評価したい

### **モード2: SAHI評価**

```bash
# 基本
uv run validate.py sahi --yolo-dataset Dataset/YOLODataset_test_with_label

# エラー解析付き
uv run validate.py sahi \
  --yolo-dataset Dataset/YOLODataset_test_with_label \
  --error-analysis \
  --export-visuals
```

**いつ使う?**
- ⚠️ 相対比較のみ（絶対値は信頼できない）
- ✅ エラー解析で改善点を特定
- ✅ スライス推論の実験

### **モード3: 比較モード（⭐ 最もおすすめ）**

```bash
uv run validate.py compare \
  --yolo-dataset Dataset/YOLODataset_test_with_label \
  --name comprehensive_eval \
  --error-analysis \
  --export-visuals
```

**出力ディレクトリ:**
```
runs/val/comprehensive_eval/
├── ultralytics/              # Ultralytics評価結果
│   ├── metrics.json
│   ├── predictions.json
│   └── ...
├── sahi/                     # SAHI評価結果
│   ├── eval.json
│   ├── predictions.json
│   └── segm/                # エラー解析プロット
│       ├── segm-pod-allarea.png
│       └── ...
└── (自動生成される比較サマリー)
```

**いつ使う?**
- ✅ **常にこれを使うべき**
- ✅ 公式精度 + エラー解析を一度に取得
- ✅ 全体像を把握したい

---

## 📊 あなたのモデルの評価結果

### **最新の評価（Ultralytics公式）**

```
Segmentation mAP50-95: 36.27%  ✅ 良好レベル
                                  （商用利用可能）

【評価基準】
0-20%  : 🔴 要改善
20-40% : 🟡 まぁまぁ
40-60% : 🟢 良好          ← あなたは実はここ！
60-80% : 🔵 優秀
80%+   : 🟣 最高レベル
```

### **改善ポテンシャル（エラー解析より）**

1. **BBox精度の向上**: +33.8%のポテンシャル
   - よりタイトなアノテーション
   - 学習データの増強

2. **偽陽性の削減**: +9.6%のポテンシャル
   - 信頼度閾値の調整
   - ネガティブサンプルの追加

3. **偽陰性**: 0% ← 完璧！

---

## 🔄 既存スクリプトからの移行

### **移行前**
```bash
# 3つの別々のスクリプト
python ultralytics_val.py --model best.pt --data data.yaml
python SAHI_val.py --yolo-dataset Dataset/YOLODataset_test_with_label
python diagnose_difference.py ...
```

### **移行後（推奨）**
```bash
# 1つのコマンドで全て実行
uv run validate.py compare \
  --yolo-dataset Dataset/YOLODataset_test_with_label \
  --error-analysis
```

### **互換性**
既存のスクリプトも引き続き使用可能です：
```bash
python SAHI_val.py --yolo-dataset Dataset/YOLODataset_test_with_label
python ultralytics_val.py
```

---

## 💡 ベストプラクティス

### **日常的な評価**
```bash
# クイックチェック（高速）
uv run validate.py ultralytics
```

### **詳細な分析**
```bash
# 包括的な評価（推奨）
uv run validate.py compare \
  --yolo-dataset Dataset/YOLODataset_test_with_label \
  --error-analysis
```

### **実験比較**
```bash
# 実験1
uv run validate.py compare --name exp1 --error-analysis

# 実験2（異なる設定）
uv run validate.py compare --name exp2 --error-analysis

# 結果はそれぞれ別ディレクトリに保存される
# runs/val/exp1/
# runs/val/exp2/
```

---

## 🎯 重要な注意事項

### ✅ **DO（やるべきこと）**

1. **Ultralyticsの値を信頼する**
   ```
   Ultralytics: 36.27% ← これを使う！
   ```

2. **SAHIはエラー解析のみに使う**
   ```
   SAHI: 12.90% ← 絶対値は無視
   エラー解析プロット ← これを見る！
   ```

3. **比較モードを常に使う**
   ```bash
   uv run validate.py compare ...
   ```

### ❌ **DON'T（やってはいけないこと）**

1. **SAHIの絶対値を信じない**
   ```
   SAHI: 12.90% ← この値で精度を判断しない
   ```

2. **単独でSAHIだけを実行しない**
   ```bash
   # これだけだと誤解する可能性
   uv run validate.py sahi ...
   ```

---

## 📚 参考リソース

- `validation/README.md` - モジュールの詳細ドキュメント
- `validate.py --help` - CLIヘルプ
- 既存スクリプト（互換性のため維持）:
  - `SAHI_val.py`
  - `ultralytics_val.py`
  - `diagnose_difference.py`

---

## 🎊 まとめ

### **新しいワークフロー**

```bash
# ステップ1: 包括的な評価を実行
uv run validate.py compare \
  --yolo-dataset Dataset/YOLODataset_test_with_label \
  --error-analysis

# ステップ2: 結果を確認
# - Ultralytics: 公式精度（信頼できる）
# - SAHI: エラー解析（改善ポイント）

# ステップ3: 改善を実施
# - アノテーションの見直し
# - データ拡張の強化
# - ハイパーパラメータの調整

# ステップ4: 再評価
uv run validate.py compare --name exp2 --error-analysis
```

### **あなたのモデルは良好です！**

✅ **現在の精度**: 36.27% mAP（商用利用可能レベル）  
🎯 **改善ポテンシャル**: +43%（BBox精度 + 偽陽性削減）  
💪 **次のステップ**: エラー解析を見て、優先順位の高い改善から着手

---

**整理・統合版の評価システムを活用して、より良いモデルを作りましょう！** 🚀

